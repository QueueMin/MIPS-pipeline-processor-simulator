...
public:
  ...
  void ID()
  {
    ...
    for (int i = 0; i < 6; i++)
      Operation[i] = this->IFID.Inst[i + 26];
    for (int i = 0; i < 5; i++)
      Rs[i] = this->IFID.Inst[i + 21];
    for (int i = 0; i < 5; i++)
      Rt[i] = this->IFID.Inst[i + 16];
    for (int i = 0; i < 5; i++)
      Rd[i] = this->IFID.Inst[i + 11];
    for (int i = 0; i < 6; i++)
      Function[i] = this->IFID.Inst[i];
    for (int i = 0; i < 16; i++)
      Extend[i] = this->IFID.Inst[i];
    for (int i = 0; i < 26; i++)
    ...
    if (ForwardUnit.BranchForwardA == 0)
      BranchCompareData1 = IDEX.Data1;
    else if (ForwardUnit.BranchForwardA == 1)
      BranchCompareData1 = ForwardUnit.MEMWBData;
    else if (ForwardUnit.BranchForwardA == 2)
      BranchCompareData1 = ForwardUnit.EXMEMData;

    if (ForwardUnit.BranchForwardB == 0)
      BranchCompareData2 = IDEX.Data2;
    else if (ForwardUnit.BranchForwardB == 1)
  ...
  void EX()
  {
    ...
    //set Data1
    if (ForwardUnit.forwardA == 0)
      ALUin1 = IDEX.Data1;
    else if (ForwardUnit.forwardA == 1)
      ALUin1 = ForwardUnit.MEMWBData;
    else if (ForwardUnit.forwardA == 2)
      ALUin1 = ForwardUnit.EXMEMData;

    //set Data2
    if (ForwardUnit.forwardB == 0)
      ALUin2 = IDEX.Data2;
    else if (ForwardUnit.forwardB == 1)
      ALUin2 = ForwardUnit.MEMWBData;
    else if (ForwardUnit.forwardB == 2)
      ALUin2 = ForwardUnit.EXMEMData;
    
    //sw를 위한 forwarding된 값 저장
    EXMEM.Data2 = ALUin2;
  ...
  void runSingleCycle()
  {
    if (this->MManager.getFileLength() == -1)
      return;
    WB();
    MEM();
    EX();
